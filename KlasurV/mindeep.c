...
int min_leaf_level(Tree* n, int level) {
   ...
   if(!n->left)return 1 + min_leaf_level(n->right,level + 1);
   if(!n->right)return 1  + min_leaf_level(n->left,level +  1);

   if (min_leaf_level(n->left, level + 1) < min_leaf_level(n->right, level + 1))return 1 + min_leaf_level(n->left, level + 1);
   return  1  + min_leaf_level(n->right, level + 1);
}
