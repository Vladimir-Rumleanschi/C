...
int max_path_prod_rec(Tree *t, int max_so_far, int path_prod)
{
    ...
    if(is_leaf(t) && max_so_far < 0){
        path_prod = path_prod * t->value;
        max_so_far = 0;
    }

    if(t->left != NULL && t->right == NULL){
        path_prod = path_prod * max_path_prod_rec(t->left, -1, path_prod);
    }

     if(t->left == NULL && t->right != NULL){
        path_prod = path_prod * max_path_prod_rec(t->left, max_so_far -1, path_prod);
    }

    if(t->left != NULL && t->right != NULL ){
        if(t->left->value < t->right->value){
            path_prod = path_prod * max_path_prod_rec(t->right,max_so_far + 1, path_prod);
        }else{
            path_prod = path_prod * max_path_prod_rec(t->left, max_so_far + 1, path_prod);
        }
    }
