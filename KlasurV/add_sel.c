...
bool chechForOperator(String order){
    bool retWert = false;
...
{
    // implement
    if(!chechForOperator(path) || t == NULL || path[0] == '\0')return 0;
    if(path[0] == 'r'){
        path++;
        sum = sum + add_selected(t->right, path , sum);
    }else if(path[0] == 'l'){
        path++;
        sum = sum + add_selected(t->left, path , sum);
    }else if(path[0] == '+'){
        path++;
        sum = sum + t->value +  add_selected(t, path , sum);
    } 
    return sum;
}
